...
public class SyncController {
    ...
    public ResponseEntity<?> retryFailedSyncs() {
        int retriedCount = syncService.retryFailedSyncs();
        return ResponseEntity.ok(new ApiResponse<>(
            "Retried " + retriedCount + " failed syncs", 
            Map.of("retriedCount", retriedCount)
        ));
    }
    ...
    public ResponseEntity<?> checkSyncHealth() {
        Map<String, Object> health = syncService.checkSyncHealth();
